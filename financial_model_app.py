...
def save_scenario(name):
    # Define model_params, lead_params, and gmv_params here
    model_params = {
    ...
    st.session_state.scenarios[name] = {
        "params": {
            k: (v * 100 if k.endswith("_rate") and k != "take_rate" else v)
            for k, v in model_params.items()
        },
        "lead_params": {
            k: (v * 100 if k.endswith("_rate") or k == "referral_probability" else v)
            for k, v in lead_params.items()
        },
        "gmv_params": {
            k: (v * 100 if k in ["feature_penetration", "take_rate"] else v)
            for k, v in gmv_params.items()
        },
    }
...
st.session_state.churn_rate = (
    st.sidebar.slider(
        ...
        0.0,
        20.0,
        float(st.session_state.get("churn_rate", 0.03) * 100),  # Multiply by 100 here
        0.1,
    )
...
st.session_state.users_per_customer = st.sidebar.number_input(
    "Users per Customer", value=st.session_state.get("users_per_customer", 3), step=1
)
st.session_state.gmv_per_user = st.sidebar.number_input(
    "GMV per User ($)", value=st.session_state.get("gmv_per_user", 300000), step=10000
)
...
fig_customers.add_trace(
    go.Scatter(
        x=results["Date"], y=results["Customers"].round().astype(int), name="Customers"
    )
)
...
new_customers_monthly = (
    customers_by_source.diff().fillna(customers_by_source.iloc[0]).round().astype(int)
)
...
# Display the first month's closed won value for comparison
first_month_closed_won = pipeline_results.iloc[0]["closed_won"].round().astype(int)
st.write(f"First Month Closed Won: {first_month_closed_won}")

# Optional: Display average monthly closed won for context
avg_monthly_closed_won = average_pipeline_results["closed_won"]
st.write(f"Average Monthly Closed Won: {avg_monthly_closed_won}")

# New table: Final Customers by Channel
st.subheader("Final Customers by Acquisition Channel")
final_customers = customers_by_source.iloc[-1].round().astype(int)
final_customers_df = pd.DataFrame(
    {
        "Channel": final_customers.index,
        "Number of Customers": final_customers.values,
        "Percentage": (final_customers.values / final_customers.sum() * 100).round(2),
    }
)
final_customers_df = final_customers_df.sort_values(
    "Number of Customers", ascending=False
).reset_index(drop=True)
st.table(final_customers_df)
...
if scenarios_to_compare:
    ...
    for scenario in scenarios_to_compare:
        ...
        scenario_results, _, _ = cached_run_model(
            ...
            scenario_data["lead_params"],
            scenario_data["gmv_params"],
        )
    ...
    fig_mrr_comparison.update_layout(title="MRR Comparison", yaxis_title="MRR ($)")
...
# Add this at the beginning of your app, after loading scenarios
st.write(f"Loaded scenarios: {list(st.session_state.scenarios.keys())}")
